...
long long int bsearch (long long int vector[], long long int start, long long int end, long long int searching_for) {
	...
	else if (vector[mid] == searching_for) return mid;
	else if (vector[mid] > searching_for) return bsearch(vector, start, mid - 1, searching_for);
	else return bsearch(vector, mid + 1, end, searching_for);
}
